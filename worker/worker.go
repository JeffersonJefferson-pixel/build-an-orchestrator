...
func (w *Worker) UpdateTasks() {
	// run continuous loop to update worker's tasks state.
	for {
		log.Printf("Checking status of tasks")
		w.updateTasks()
...
func (w *Worker) updateTasks() {
	// iterate over tasks in worker db.
	for id, t := range w.Db {
		// inspect task if it is running
		if t.State == task.Running {
			...
			if resp.Container == nil {
				log.Printf("No container running task %s\n", id)
